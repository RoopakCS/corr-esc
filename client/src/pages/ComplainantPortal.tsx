...
export function ComplainantPortal() {
  ...
  const [contestingIncidentId, setContestingIncidentId] = useState<string | null>(null);
              ...
              <div className="space-y-4">
                {complaints.map((complaint) => (
                  ...
                  >
                    ...
                    {/* Resolution Verification Grace Period Prompt */}
                    {complaint.incident?.status === "Resolved" && (
                      <div className="p-4 bg-purple-950/40 border-2 border-purple-500/40 rounded-xl space-y-3">
                        ...
                        <div className="flex flex-wrap items-center gap-2 pt-1">
                          <button
                            type="button"
                            onClick={() => handleConfirmResolution(complaint.incidentId)}
                            disabled={actionLoading}
                            className="inline-flex items-center gap-1.5 px-3.5 py-2 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white rounded-lg text-xs font-semibold shadow transition"
                          >
                          ...
                          <button
                            type="button"
                            onClick={() => handleContest(complaint.incidentId)}
                            disabled={actionLoading}
                            className="inline-flex items-center gap-1.5 px-3.5 py-2 bg-rose-600 hover:bg-rose-500 disabled:opacity-50 text-white rounded-lg text-xs font-semibold shadow transition"
                          >
                        ...
                        {contestingIncidentId === complaint.incidentId && (
                          <div className="pt-2 space-y-2">
                            <input
                              type="text"
                              value={contestFeedback[complaint.incidentId] || ""}
                              onChange={(e) =>
                                setContestFeedback({
                                  ...contestFeedback,
                                  [complaint.incidentId]: e.target.value,
                                })
                              }
                              placeholder="Optional explanation of why the issue is still not fixed..."
                              className="w-full bg-slate-900 border border-purple-500/40 rounded-lg px-3 py-1.5 text-xs text-white focus:outline-none focus:ring-2 focus:ring-rose-500"
                            />
                            <div className="flex justify-end gap-2">
                              <button
                                type="button"
                                onClick={() => setContestingIncidentId(null)}
                    ...
                    {/* Previous Reopen Penalty Note */}
                    {Boolean(complaint.incident?.reopenCount && complaint.incident.reopenCount > 0) && (
                      <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-rose-500/10 border border-rose-500/20 text-rose-300 text-xs">
                        <AlertCircle className="w-3.5 h-3.5 text-rose-400 flex-shrink-0" />
                        <span>This incident was previously contested and reopened ({complaint.incident?.reopenCount} time{(complaint.incident?.reopenCount ?? 0) > 1 ? "s" : ""}) with an escalation penalty.</span>
                      </div>
                    )}
